...
void mostrarPor(Map *mapaCanciones, char *mensaje){
  char texto[100];
  printf("Ingrese el %s de la canción: ", mensaje);
  scanf(" %[^\n]", texto);
  ...
  if (pair != NULL){
      ...
      while (cancion != NULL){
        ...
        //if (list_next(canciones) == NULL && contador == -1) break;
      }
    printf("===========================================================================\n");
...
void agregarCancionALista(Map *mapaListas, Map *songsById)
{
  ...
  //listaReproduccion * listaPrueba = lista->canciones; //list_pushBack(lista, cancion);
  list_pushBack(lista->canciones, cancion);
  
  //list_pushBack(lista->canciones, cancion);
  printf("Se agregó la cancion %s a la lista\n", cancion->trackName);
